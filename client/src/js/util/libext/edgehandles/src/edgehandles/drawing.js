...
function setHandleFor (node) {
  ...
  const handlePosition = typeof options.handlePosition === typeof '' ? () => options.handlePosition : options.handlePosition
  ...
  // grab axes
  const axes = handlePosition(node).toLowerCase().split(/\s+/)
  const axisX = axes[0]
  const axisY = axes[1]
  ...
  const hx = this.hx = p.x + moveX
  const hy = this.hy = p.y + moveY
  const pos = { x: hx, y: hy }
