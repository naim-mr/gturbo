...
const printRule = (event) => {
  ...
  let n = 0
  if (id.length == 2) { n = parseInt((id).slice(1, 2)) } else { n = parseInt(id.slice(8, 9)) }
...
const onSwitchInclusionRule = (event) => {
  ...
  let n = 0
  vue.show()
  if (id.length == 1) { n = parseInt(id) } else { n = parseInt(id.slice(7, 8)) }
  vue.switch(n)
}
const onSwitchRule = (event) => {
  if (lastButton != null) { lastButton.classList.remove('selectedButton') }
  const id = event.target.id
  vue.show()
  let n = 0
  if (id.length == 1) { n = parseInt(id) } else { n = parseInt(id.slice(7, 8)) }
